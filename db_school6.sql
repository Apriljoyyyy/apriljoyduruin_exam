...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_Add_Class` (IN `InputCourseID` INT, IN `InputInstructorID` INT, IN `InputRoomID` INT, IN `InputSchedule` VARCHAR(50))  BEGIN
    INSERT INTO product (CourseID, InstructorID, RoomID, Schedule)
    VALUES (InputCourseID, InputInstructorID, InputRoomID, InputSchedule);
END$$
